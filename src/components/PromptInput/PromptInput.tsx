...
function PromptInput({
  ...
  const hasTungstenSession = useAppState(s => "ant" === 'ant' && s.tungstenActiveSession !== undefined);
  const tmuxFooterVisible = "ant" === 'ant' && hasTungstenSession;
  ...
  const hasBgTaskPill = useMemo(() => Object.values(tasks).some(t => isBackgroundTask(t) && !("ant" === 'ant' && isPanelAgentTask(t))), [tasks]);
  ...
  const tasksFooterVisible = (runningTaskCount > 0 || "ant" === 'ant' && coordinatorTaskCount > 0) && !shouldHideTasksFooter(tasks, showSpinnerTree);
  ...
  useKeybindings({
    'footer:up': () => {
      ...
      if (tasksSelected && "ant" === 'ant' && coordinatorTaskCount > 0 && coordinatorTaskIndex > minCoordinatorIndex) {
    ...
    'footer:down': () => {
      ...
      if (tasksSelected && "ant" === 'ant' && coordinatorTaskCount > 0) {
