// biome-ignore-all lint/complexity/useRegexLiterals: terminal escape-sequence patterns use RegExp(String.raw...) to avoid control-character literal lint
/**
...
const META_KEY_CODE_RE = new RegExp(String.raw`^(?:\x1b)([a-zA-Z0-9])$`)

const FN_KEY_RE = new RegExp(
  String.raw`^(?:\x1b+)(O|N|\[|\[\[)(?:(\d+)(?:;(\d+))?([~^$])|(?:1;)?(\d+)?([a-zA-Z]))`,
)
...
const CSI_U_RE = new RegExp(String.raw`^\x1b\[(\d+)(?:;(\d+))?u`)
...
const CURSOR_POSITION_RE = new RegExp(String.raw`^\x1b\[\?(\d+);(\d+)R$`)
// OSC response: OSC code ; data (BEL|ST)
const OSC_RESPONSE_RE = new RegExp(
  String.raw`^\x1b\](\d+);(.*?)(?:\x07|\x1b\\)$`,
  's',
)
